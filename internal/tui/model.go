...
func NewModel() Model {
	running := list.New(nil, list.NewDefaultDelegate(), 0, 0)
...
func (m Model) handleKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	//
	//   - F2 returns keyboard focus to the list, client-side only — the
	//     session itself is left running (still fed live in the
	...
	//     at"), so repurposing one here changes nothing about what the
	//     agent used to receive — zero collision risk, unlike Esc. Ctrl-P
	//     Ctrl-Q is still forwarded as two ordinary bytes and still works
	//     as the "real" server-side detach (internal/control/stream.go's
	//     existing detachScanner, ending the stream exactly like `muro
	//     sandbox attach` — paneTickMsg, above, is what notices that and
	//     flips focus back too), but a real terminal or multiplexer sitting
	//     between the operator and this process can intercept or remap a
	//     two-key control-character chord in ways outside muro's control —
	//     which is exactly why Esc was tried as a single-key alternative
	//     before this fix. F2 keeps that single-key reliability without
	//     Esc's collision problem.
...
func (m Model) viewSplitPane() string {
	...
	return lipgloss.JoinHorizontal(lipgloss.Top, m.running.View(), "  ", pane)
}
...
func (m Model) footerHint() string {
	...
	if m.focus == focusConsole {
		return "typing goes to " + m.sessTarget + "  ·  f2: back to the list  ·  pgup/pgdn or wheel: scroll  ·  ctrl-p ctrl-q: end session"
	}
	return "↑/↓: highlight (console follows)  ·  enter: type into it (or start, if stopped)  ·  s: stop  ·  tab: switch pane  ·  q: quit"
}
